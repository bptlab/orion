...
def graphviz_visualization(activities_count, dfg, att_config, image_format="png", measure="frequency",
    ...
    viz.attr(nodesep='0.25') # minimum space between two nodes
    viz.attr(ranksep='0.2') # minimum vertical distance between the bottom of the nodes on one rank and the tops of nodes in the next
   #viz.attr(pad='0.25')
    ...
    #print(dfg_edges)
    #print(list(att_config.keys()))
    for rel in list(att_config.keys()):
        split = rel.split(":")
        ele = (split[0], split[1])
        if (ele not in dfg_edges):
            dfg_edges.append(ele)
    ...
    for edge in dfg_edges:
        ...
        rel = act1 + ":" + act2
        try:
            att_conf = att_config[rel]
            for soft_corr in att_conf["soft"]:
                    corr_info = att_conf["soft"][soft_corr]
                    soft_act_1 = corr_info["Act_1"]
                    soft_act_2 = corr_info["Act_2"]
                    act_1_soft.append(soft_act_1)
                    act_2_soft.append(soft_act_2)
        except:
            pass
    # represent edges
    for edge in dfg_edges:
        ...
        if(len(att_conf) > 0):
            Mean1 = att_conf["M1"]
            Mean2 = att_conf["M2"]
            att_label = att_conf["E_At"]
            RBC = att_conf["RBC"]
            Directly = att_conf["Directly"]
            N = att_conf["N"]
            label = att_label + " (" + str(RBC) + ")\\n"# + "N: " +  str(N)
            #add strong correlation at same relation
            try:
                for strong_corr in att_conf["strong"]:
                    corr_info = att_conf["strong"][strong_corr]
                    measure_to_add = ""
                    coeff_to_add = ""
                    if corr_info["EA_1"] != att_label:
                        measure_to_add = corr_info["EA_1"]
                    else:
                        measure_to_add = corr_info["EA_2"]
                    coeff_to_add = str(round(corr_info["scipy_corr"], 2))
                    label = label + measure_to_add + ": " + coeff_to_add + "\\n"   
            except:
                pass 
            #add soft correlation (Wilcoxon-test) at another relation   
            try:
                for soft_corr in att_conf["soft"]:
                    corr_info = att_conf["soft"][soft_corr]
                    soft_act_1 = corr_info["Act_1"]
                    soft_act_2 = corr_info["Act_2"]
                    soft_rbc = str(- round(corr_info["RBC"], 2))
                    soft_label = att_label + ": " + soft_rbc + "\\n" 
                    if(- corr_info["RBC"] > 0):
                        if(corr_info["Directly"] == 1):
                            viz.edge(str(hash(soft_act_1)), str(hash(soft_act_2)), label=soft_label, penwidth="3", fontsize=font_size, color="red", fontname="times bold italic")
                        else:
                            viz.edge(str(hash(soft_act_1)), str(hash(soft_act_2)), label=soft_label, penwidth="3", fontsize=font_size, color="red", style="dotted", fontname="times bold italic")
                    else:
                        if(corr_info["Directly"] == 1):
                            viz.edge(str(hash(soft_act_1)), str(hash(soft_act_2)), label=soft_label, penwidth="3", fontsize=font_size, color="blue", fontname="times bold italic")
                        else:
                           viz.edge(str(hash(soft_act_1)), str(hash(soft_act_2)), label=soft_label, penwidth="3", fontsize=font_size, color="blue", style="dotted", fontname="times bold italic") 
            except:
                pass          
            ...
            if(RBC > 0):
                if(Directly == 1):
                    viz.edge(str(hash(edge[0])), str(hash(edge[1])), label=label, taillabel=str(Mean1), headlabel=str(Mean2), penwidth="3", fontsize=font_size, color="red", fontname="times bold")
                else:
                    #label = att_label + " (" + str(RBC) + ")\\n" 
                    viz.edge(str(hash(edge[0])), str(hash(edge[1])), label=label, taillabel=str(Mean1), headlabel=str(Mean2), penwidth="3", fontsize=font_size, color="red", style="dotted", fontname="times bold")
            else:
                if(Directly == 1):
                    viz.edge(str(hash(edge[0])), str(hash(edge[1])), label=label, taillabel=str(Mean1), headlabel=str(Mean2), penwidth="3", fontsize=font_size, color="blue", fontname="times bold")
                else:
                    #label = att_label + " (" + str(RBC) + ")\\n" 
                    viz.edge(str(hash(edge[0])), str(hash(edge[1])), label=label, taillabel=str(Mean1), headlabel=str(Mean2), penwidth="3", fontsize=font_size, color="blue", style="dotted", fontname="times bold")


        else:
            if act1 in act_1_soft and act2 in act_2_soft:
                pass
            else:
                label = str(dfg[edge])
                viz.edge(str(hash(edge[0])), str(hash(edge[1])), label=label, penwidth="1", fontsize=font_size, color="black")
    ...
    if start_activities_to_include:
        viz.node("@@startnode", "<&#9679;>", shape='circle', fontsize="12")
    ...
    if end_activities_to_include:
        # <&#9632;>
        viz.node("@@endnode", "<&#9632;>", shape='doublecircle', fontsize="12")
...
def apply(dfg: Dict[Tuple[str, str], int], att_config : None, log: EventLog = None, parameters: Optional[Dict[Any, Any]] = None, activities_count : Dict[str, int] = None, soj_time: Dict[str, float] = None) -> Digraph:
    """
    ...
    start_activities = exec_utils.get_param_value("START_ACTIVITIES", parameters, {})
    end_activities = exec_utils.get_param_value("END_ACTIVITIES", parameters, {})
    ...
    return graphviz_visualization(activities_count, dfg, att_config=att_config, image_format=image_format, measure="frequency",
