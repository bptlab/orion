...
def node_mappings_from_gviz(gviz):
    j = json.loads(gviz.pipe('json').decode())

    node_id_mapping = {}
    for obj in j['objects']:
        id = obj['name']
        label = obj['label']
        label_split = label.rsplit(' (', 1)
        if len(label_split) == 2:
            name = label_split[0]
            node_id_mapping[id] = name

    return node_id_mapping


def gviz_to_html(gviz):
    tree = etree.XML(gviz.pipe('svg'))

    gtags = tree.xpath("//*[name()='g']")
    for g in gtags:
        titletag = g.xpath("./*[name()='title']")
        if titletag:
            subtags = g.xpath("./*")
            id = titletag[0].text
            ...
            if g.get("class") == "node":
                for subtag in subtags:
                    # modify subtag
                    subtag.set('data-type', "node")
                    subtag.set('data-id', id)
            else:
                for subtag in subtags:
                    # modify subtag
                    subtag.set('data-type', "edge")
                    subtag.set('data-id-left', id.split("->")[0])
                    subtag.set('data-id-right', id.split("->")[1])
    ...
    html = f"""<!DOCTYPE html>
                    ...
                    <body>
                        <div id="graph">{etree.tostring(tree, encoding=str)}</div>
                    </body>
                    </html>
                    """
    return html
...
def get_trans_color(base_color, trans_count, min_trans_count, max_trans_count):
    """
    ...
    trans_base_color = int(
        base_color - 100 * (trans_count - min_trans_count) / (max_trans_count - min_trans_count + 0.00001))
    trans_base_color_hex = str(hex(trans_base_color))[2:].upper()
    return "#" + trans_base_color_hex + trans_base_color_hex + "FF"


def get_activities_color(activities_count, base_color):
    """
    ...
    for ac in activities_count:
        ...
        v1 = get_trans_color(base_color, v0, min_value, max_value)
...
def graphviz_visualization(activities_count, dfg, image_format="png", measure="frequency",
                           ...
                           end_activities=None, soj_time=None, font_size="6",
                           bgcolor="transparent", stat_locale: dict = None,
                           activities_RBC: Dict[str, float] = None, edges_RBC: Dict[Tuple[str, str], float] = None):
    """
    ...
    viz = Digraph("", filename=filename.name, engine='dot', graph_attr={'bgcolor': bgcolor, 'scale': '0.4'},
                  node_attr={'height': '0.4'})

    # first, r emove edges in diagram that exceeds the maximum number of edges in the diagram
    dfg_key_value_list = []
    for edge in dfg:
    ...
    activities_color = get_activities_color(activities_RBC, 255)
    # represent nodes
    ...
    edges_color = get_activities_color(edges_RBC, 200)
    ...
    for edge in dfg_edges:
        ...
        viz.edge(str(hash(edge[0])), str(hash(edge[1])), label=label, penwidth=str(penwidth[edge]),
                 color=edges_color[edge], fontsize=font_size)
    ...
    if start_activities_to_include:
        viz.node("@@startnode", "<&#9679;>", shape='circle', fontsize="21")
    ...
    if end_activities_to_include:
        # <&#9632;>
        viz.node("@@endnode", "<&#9632;>", shape='doublecircle', fontsize="20")
...
def apply(dfg: Dict[Tuple[str, str], int], log: EventLog = None, parameters: Optional[Dict[Any, Any]] = None,
          activities_count: Dict[str, int] = None, soj_time: Dict[str, float] = None,
          activities_RBC: Dict[str, float] = None, edges_RBC: Dict[str, float] = None) -> Digraph:
    """
    ...
    return graphviz_visualization(activities_count, dfg, image_format=image_format, measure="frequency",
                                  ...
                                  activities_RBC=activities_RBC, edges_RBC=edges_RBC)
